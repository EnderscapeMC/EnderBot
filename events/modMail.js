...
const client = new Client({
    ...
    presence: {
        activities: [{
            name: "DM me to appeal a punishment.",
            type: 1,
            url: "www.enderscape.net"
        }]
    },
    shards: "auto"
});

const sql = 'INSERT INTO mail (author, channelId) VALUES (?, ?)';
connection.query(sql, [data.author, data.channelId], (error, results) => {
    if (error) {
...
client.on('messageCreate', async (message) => {
    ...
    const channel = guild.channels.cache.find((x) => x.id === sql.mails.findOne((v) => v.author === message.author.id)?.channelId && x.parentId === category.id);
    ...
    if (message.channel.type == ChannelType.DM) {
        ...
        if (!channel) {
            ...
            sql.mails.set({ author: message.author.id, channelId: channel.id });

            let embedDM = new Embesqluilder()
        ...
        const requestedUserMail = guild.members.cache.find((x) => x.user.id === sql.mails.findOne((v) => v.channelId === message.channelId)?.author);
...
client.on('interactionCreate', async (interaction) => {
    if (interaction.isButton()) {
        ...
        if (ID == "close_dm") {
            ...
            const channelRECHECK = guild.channels.cache.find(x => x.id === sql.mails.findOne((v) => v.author === interaction.user.id)?.channelId && x.parentId === category.id);
    ...
    if (interaction.isModalSubmit()) {
        ...
        if (ID == "modal_close") {
            ...
            const requestedUserMail = guild.members.cache.find((x) => x.user.id === sql.mails.findOne((v) => v.channelId === interaction.channelId)?.author);
            ...
            sql.mails.findOneAndDelete((v) => v.channelId === interaction.channelId);
...
client.on('channelDelete', async (channel) => {
    if (channel.parentId === channel.guild.channels.cache.find((cat) => cat.id === config.modmail.categoryId || cat.name === "ModMail")?.id) {
        const author = client.users.cache.get(sql.mails.findOne((v) => v.channelId === channel.id)?.author);

        sql.mails.findOneAndDelete((v) => v.channelId === channel.id);
